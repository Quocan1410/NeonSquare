...
export function CommentModal({ isOpen, onClose, post }: CommentModalProps) {
  ...
  const handleSubmitReply = async () => {
    if (!replyContent.trim() || !user || !replyingTo) {
      console.log('Missing required data for reply:', { replyContent, user, replyingTo });
      return;
    }

    console.log('Submitting reply:', { commentId: replyingTo.id, content: replyContent, userId: user.id });
    setIsSubmittingReply(true);
    try {
      const result = await apiService.createReply(replyingTo.id, replyContent, user.id);
        ...
        <div className="p-4 border-b border-slate-200 dark:border-slate-700">
          <div className="flex items-start space-x-3">
            <Avatar className="w-10 h-10">
              ...
              <AvatarFallback className={`bg-gradient-to-br ${getAvatarColors(post.author?.id || 'default')} text-white`}>
                ...
                <div key={comment.id} className="group">
                  <div className="flex items-start space-x-4">
                    <Avatar className="w-10 h-10 ring-2 ring-slate-200 dark:ring-slate-700 group-hover:ring-blue-400 transition-all duration-200">
                      ...
                      />
                      <AvatarFallback className={`bg-gradient-to-br ${getAvatarColors(comment.author?.id || 'default')} text-white font-semibold`}>
                                  ...
                                  <div className="flex items-start space-x-3">
                                    <Avatar className="w-8 h-8 ring-2 ring-slate-200 dark:ring-slate-700">
                                      <AvatarImage 
                                        ...
                                        className="object-cover"
                                      />
                                      <AvatarFallback className={`bg-gradient-to-br ${getAvatarColors(reply.author?.id || 'default')} text-white font-semibold text-xs`}>
                                        {reply.author ? `${reply.author.firstName.charAt(0)}${reply.author.lastName.charAt(0)}` : 'U'}
                                      </AvatarFallback>
                                    </Avatar>
                                    <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-start space-x-3">
                            <Avatar className="w-8 h-8 ring-2 ring-slate-200 dark:ring-slate-700">
                              <AvatarImage 
                                ...
                                className="object-cover"
                              />
                              <AvatarFallback className={`bg-gradient-to-br ${getAvatarColors(user?.id || 'default')} text-white font-semibold text-xs`}>
                            ...
                            <div className="flex-1 relative">
                              <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-600 shadow-sm focus-within:shadow-md focus-within:border-blue-400 dark:focus-within:border-blue-500 transition-all duration-200">
                                <textarea
                                  placeholder={`Reply to ${comment.author?.firstName || 'Unknown User'}...`}
          ...
          <div className="flex items-start space-x-4">
            <Avatar className="w-10 h-10 ring-2 ring-slate-200 dark:ring-slate-700">
              <AvatarImage 
                ...
                className="object-cover"
              />
              <AvatarFallback className={`bg-gradient-to-br ${getAvatarColors(user?.id || 'default')} text-white font-semibold`}>
                {user ? `${user.firstName.charAt(0)}${user.lastName.charAt(0)}` : 'U'}
              </AvatarFallback>
            </Avatar>
            <div className="flex-1 relative">
